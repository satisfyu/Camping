package net.satisfy.camping.neoforge.client.renderer.player.layers;
...
public class LargeBackpackLayer<T extends LivingEntity, M extends HumanoidModel<T>> extends RenderLayer<T, M> {
    ...
    public LargeBackpackLayer(RenderLayerParent<T, M> renderLayerParent) {
        ...
        this.model = new LargeBackpackModel<>(Minecraft.getInstance().getEntityModels().bakeLayer(CampingClientNeoForge.LARGE_BACKPACK_LAYER));
    }
    ...
    protected ResourceLocation getTextureLocation(@NotNull T entity) {
        return Camping.identifier("textures/model/large_backpack.png");
    }
}
